// Package ckldfire is a framework that provides functions
...
// IpIncrement increments an IP adddress by 1.
...
// StrToWords returns a string which was split by spaces.
...
// RandomSelectStr returns a string array that was randomly selected from a nested list of strings
func RandomSelectStrNested(list [][]string) []string {
	rand.Seed(time.Now().UnixNano())
	return list[rand.Intn(len(list))]
}

// RandomSelectStr returns an integer that was randomly selected from a list of integer.
...
// RemoveDuplicatesStr returns an array of integers that are unique to each other.
...
// GetLocalIp is used to get the local Ip address of the current machine as a string.
...
// GetGlobalIp is used to return the global Ip address of the current machine as a string.
...
// Iface returns the currently used wireless interface and its MAC address as a string.
...
// MD5Hash hashes a given string using the MD5 algorithm.
...
// ReadFile is used to read a given file as a string.
...
// IsRoot checks if the current user has administrator privileges.
func IsRoot() bool {
	root := true
...
// CmdDirs executes commands which are mapped to a string
...
// SndboxFilePath checks if the process is being run
...
// SandboxMac is used to check if the environment's
// MAC address matches standard MAC adddresses of
...
// SandboxAll check if an environment is virtualized
...
// SandboxAlln checks if an environment is virtualized
// by testing all sandbox checks and checking if the
// number of successful checks is equal or greater to
// a given number.
...
// Reverse executes a reverse shell on a given host:port.
...
// SendDataTCP sends to a given host:port using the TCP protocol.
...
// SendDataUDP sends to a given host:port using the UDP protocol.
